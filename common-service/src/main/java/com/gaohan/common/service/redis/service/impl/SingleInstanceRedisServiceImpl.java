package com.gaohan.common.service.redis.service.impl;
...
/**
 * Created by gaohan on 2015/12/31.
 */
public class SingleInstanceRedisServiceImpl implements RedisCacheService{
    /* 缓存默认生存时间 (秒)*/
    private final int EXPIRED_SECONDS = 1800;

    private final Pool<Jedis> jedisPool;

    public SingleInstanceRedisServiceImpl(Pool<Jedis> jedisPool) {
     ...
     */
    @Override
    public Long delByPrefixKey(String keyPattern) {
     ...
     */
    @Override
    public Long zadd(String key, Map<String, Double> scoreMembers, boolean overwrite) {
        if(overwrite){//覆盖：删除所有key下面的集合
            zremrangeByRank(key, 0, -1);
        }

        if(null == scoreMembers || scoreMembers.size() == 0)
            return 0L;
    ...
    public Long expire(String key) {
        return expire(key, this.EXPIRED_SECONDS);
    }
